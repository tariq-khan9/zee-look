...
export default function FeaturedPage({ onAddToCart }: FeaturedPageProps) {
  ...
  const fetchFeaturedProducts = async () => {
    try {
      ...
      const response = await fetch(
        `/api/products?featured=true&page=${currentPage}&limit=${productsPerPage}`
      );
      if (!response.ok) {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        ...
        <nav className="flex items-center space-x-2 text-sm text-slate-600 mb-8 font-inter">
          ...
          <span>/</span>
          <span className="text-slate-800 font-medium">Featured Products</span>
        </nav>
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-slate-600 font-inter max-w-3xl mx-auto">
            ...
            These carefully curated items represent the best of what we have to offer.
          </p>
        </div>
        ...
        {products.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-slate-500 text-sm font-inter mt-2">Check back soon for our latest featured collection!</p>
          </div>
        ) : (
          <>
